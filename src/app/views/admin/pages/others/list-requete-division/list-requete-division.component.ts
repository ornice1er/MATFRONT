...
export class ListRequeteDivisionComponent implements OnInit {
  ...
  transmettreReponse() {
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item:any) => (item.typeStructure=='Division'));
      if(check.length == 0){
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item:any) => (item.typeStructure=='Division' && item.siTransmis==1));
      if(check.length != 0){
